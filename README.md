...
* EgressIntent eschews the original library's style attribute modification of a given "modal" element on egress trigger, instead leaving the exact code to execute up to the developer.
...
| --- | --- | --- | --- |
| aggressive | `boolean` | `false` | Ignores whether a cookie has been set and always fires on egress. |
| timer | `number` | `1000` | The number of milliseconds after which to attach handlers. Reduces false positives. |
| delay | `number` | `0` | The number of milliseconds after which the user has egressed the callback should trigger. Setting this allows the user to briefly egress and then return without triggering. |
...
| target | `HTMLElement` | `document.documentElement` | The element that defines the boundaries of the egress. |
